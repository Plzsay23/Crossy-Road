...
배경 플로팅
ㄴ 배경 객체 생성 도전
ㄴ 자동차가 지나다니는 좌표나 그 간격을 저장해놓고 거기에만 소환해야 할 것 같음
...
할일 : 자동차 변경, 강 객체 랜덤 생성 구현, 강 연꽃 다리 랜덤 생성 조건 추가
...
<게임 진행 방식>
         ...
         -> 2.이어하기 -> 이름 입력(이름 찾기) -> 인게임 -> 랭킹화면 -> 메인화면
메인화면 -> 3.게임설명 -> 메인화면 (사이에 여러 기믹을 소개하는 단계를 추가할 예정, 배경 객체를 완성해야 함)
         -> 4.랭킹화면 -> 메인화면
         -> 5.상점화면 -> 메인화면 (구현 미확정)
...
ㄴ (09/30 해결) Remove 계열 함수에서 범위를 벗어나고 있었음
*/
