...
할일 : 중앙 화단(?)
       ...
       Check 함수들에서 설명화면도 동시에 확인하는데 분리하는게 나을 듯
...
11/20 자동차, 몬스터, 기차 객체 상하좌우 화면밖으로 자연스럽게 사라지는 플로팅 구현
...
ㄴ (11/20일) 위아래로 움직이게 하는 Move 함수들의 기믹을 아예 바꿔버리면서 아무리 좌표가 범위를 벗어나도 더이상
            화면이 들리지 않음
